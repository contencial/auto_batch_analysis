...
def get_domain_info(sheet):
    SPREADSHEET_ID = os.environ['OLD_DOMAIN_SSID']
...
def write_batch_info(sheet, dr, ip):
    ...
    cell_list = sheet.range('C3:C' + str(len(dr) + 2))
    i = 0
    ...
    cell_list = sheet.range('D3:D' + str(len(ip) + 2))
    i = 0
    for cell in cell_list:
        cell.value = ip[i]
        i += 1
...
def do_batch_analysis(sheet):
    domain_info = get_domain_info(sheet)
    if len(domain_info) == 0:
        logger.info(f'{sheet}: No domain')
        return
    batch_info = batch_analysis(domain_info)
    write_batch_info(sheet, batch_info[0], batch_info[1])
 
### main_script ###
if __name__ == '__main__':
    try:
        do_batch_analysis('OldDomains')
        do_batch_analysis('OldDomains2')
        do_batch_analysis('OldDomains3')
        logger.info('Finish')
        exit(0)
    except Exception as err:
